...
basedir = '../met/2019 Regatta/sat/json/'
...
try:
	if counter == 0:
		list_races = Race.objects.filter(event__comp=met, date__in=DATES)
		#list_races = Race.objects.filter(pk=8321)
	else:
		list_races = Race.objects.filter(event__comp=met, date__in=DATES)[counter:]
	...
	if Race.objects.filter(event__comp=met, date__in=DATES, complete=False).count() > 0:
		logging.info("The following races incurred non-critical errors and should be reviewed manually:")
		for r in Race.objects.filter(event__comp=met, date__in=DATES, complete=False):
